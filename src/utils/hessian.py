...
def get_layers(model, use_lora=True, use_adapter=False):
    ...
    for name, module in model.named_modules():
        if use_lora:
            ...
            if ("adapter" in name) or ("head" in name): # TODO: may need to debug
                layers[name] = module
    return layers
...
def compute_hessians_quantity(model, criterion, data, target, device="cpu", state_dict = None):
    # Get parameters and gradients of corresponding layer
