...
def run_sdp_clustering(task_affinities, k):
    ...
    X_final, X_value = sdp_clustering(task_affinities, k)
...
def compute_average_density(task_affinities, assignment):
    density = 0
    for cluster_idx in assignment:
        cluster = assignment[cluster_idx]
        cluster_density = task_affinities[cluster, :][:, cluster].sum()
        density += cluster_density/(len(cluster))
    return density
...
# # %%
# for k in np.arange(50, 101, 5):
#     assignment = run_sdp_clustering(task_affinities, int(k))
#     density = compute_average_density(task_affinities, assignment)
#     print(f'k={k}, density={density}')

# # %%
# X_final, assignment = run_regularized_sdp_clustering(task_affinities, size_lam=0.2)
# density = compute_average_density(task_affinities, assignment)
# print(f'density={density}', len(assignment))